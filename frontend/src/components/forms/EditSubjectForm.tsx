...
const EditSubjectForm = ({ subject, classes, onSuccess, onCancel }: EditSubjectFormProps) => {
  ...
  useEffect(() => {
    if (subject) {
      reset({
        ...
        type: subject.type,
        classId: typeof subject.class === 'string' ? subject.class : subject.class?._id || '',
        teacherId: subject.teacher ? (typeof subject.teacher === 'string' ? subject.teacher : subject.teacher._id) : '',
  ...
  const onSubmit: SubmitHandler<SubjectFormInputs> = async (data) => {
    ...
    try {
      const response = await apiService.subjects.update(subject._id, data);
      ...
      if (response.success) {
        ...
        setFormError(response.message || response.error || 'Failed to update subject');
      }
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          <label htmlFor="teacherId" className="block text-sm font-medium text-gray-700 mb-1">Assigned Teacher</label>
          <select
            id="teacherId"
            {...register("teacherId")}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
